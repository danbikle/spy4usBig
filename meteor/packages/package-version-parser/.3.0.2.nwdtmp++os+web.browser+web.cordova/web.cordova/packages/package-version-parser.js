...
PV.lessThan = function (versionOne, versionTwo) {                                                    // 188
  return PV.compare(versionOne, versionTwo) < 0;                                                     // 189
};                                                                                                   // 190
                                                                                                     // 191
// Given a string version, returns its major version (the first section of the                       // 192
// semver), as an integer. Two versions are compatible if they have the same                         // 193
// version number.                                                                                   // 194
//                                                                                                   // 195
// versionString: valid meteor version string.                                                       // 196
PV.majorVersion = function (versionString) {                                                         // 197
  return PV.parse(versionString).major;                                                              // 198
};                                                                                                   // 199
                                                                                                     // 200
// Takes in two meteor versions. Returns 0 if equal, a positive number if v1                         // 201
// is greater, a negative number if v2 is greater.                                                   // 202
PV.compare = function (versionOne, versionTwo) {                                                     // 203
  var v1 = PV.parse(versionOne);                                                                     // 204
  var v2 = PV.parse(versionTwo);                                                                     // 205
                                                                                                     // 206
  // If the semver parts are different, use the semver library to compare,                           // 207
  // ignoring wrap numbers.  (The semver library will ignore the build ID                            // 208
  // per the semver spec.)                                                                           // 209
  if (v1.semver !== v2.semver) {                                                                     // 210
    return semver.compare(v1.semver, v2.semver);                                                     // 211
  } else {                                                                                           // 212
    // If the semver components are equal, then the one with the smaller wrap                        // 213
    // numbers is smaller.                                                                           // 214
    return v1.wrapNum - v2.wrapNum;                                                                  // 215
  }                                                                                                  // 216
};                                                                                                   // 217
                                                                                                     // 218
// Conceptually we have three types of constraints:                                                  // 219
// 1. "compatible-with" - A@x.y.z - constraints package A to version x.y.z or                        // 220
//    higher, as long as the version is backwards compatible with x.y.z.                             // 221
//    "pick A compatible with x.y.z"                                                                 // 222
//    It is the default type.                                                                        // 223
// 2. "exactly" - A@=x.y.z - constraints package A only to version x.y.z and                         // 224
//    nothing else.                                                                                  // 225
//    "pick A exactly at x.y.z"                                                                      // 226
// 3. "any-reasonable" - "A"                                                                         // 227
//    Basically, this means any version of A ... other than ones that have                           // 228
//    dashes in the version (ie, are prerelease) ... unless the prerelease                           // 229
//    version has been explicitly selected (which at this stage in the game                          // 230
//    means they are mentioned in a top-level constraint in the top-level                            // 231
//    call to the resolver).                                                                         // 232
var parseSimpleConstraint = function (constraintString) {                                            // 233
  if (! constraintString) {                                                                          // 234
    throw new Error("Non-empty string required");                                                    // 235
  }                                                                                                  // 236
                                                                                                     // 237
  var type, versionString;                                                                           // 238
                                                                                                     // 239
  if (constraintString.charAt(0) === '=') {                                                          // 240
    type = "exactly";                                                                                // 241
    versionString = constraintString.substr(1);                                                      // 242
  } else {                                                                                           // 243
    type = "compatible-with";                                                                        // 244
    versionString = constraintString;                                                                // 245
  }                                                                                                  // 246
                                                                                                     // 247
  // This will throw if the version string is invalid.                                               // 248
  PV.getValidServerVersion(versionString);                                                           // 249
                                                                                                     // 250
  return { type: type, versionString: versionString };                                               // 251
};                                                                                                   // 252
                                                                                                     // 253
                                                                                                     // 254
// Check to see if the versionString that we pass in is a valid meteor version.                      // 255
//                                                                                                   // 256
// Returns a valid meteor version string that can be included in the                                 // 257
// server. That means that it has everything EXCEPT the build id. Throws if the                      // 258
// entered string was invalid.                                                                       // 259
PV.getValidServerVersion = function (meteorVersionString) {                                          // 260
  return PV.parse(meteorVersionString).version;                                                      // 261
};                                                                                                   // 262
                                                                                                     // 263
PV.VersionConstraint = function (vConstraintString) {                                                // 264
  var alternatives;                                                                                  // 265
  // If there is no version string ("" or null), then our only                                       // 266
  // constraint is any-reasonable.                                                                   // 267
  if (! vConstraintString) {                                                                         // 268
    // .versionString === null is relied on in the tool                                              // 269
    alternatives =                                                                                   // 270
      [ { type: "any-reasonable", versionString: null } ];                                           // 271
    vConstraintString = "";                                                                          // 272
  } else {                                                                                           // 273
    // Parse out the versionString.                                                                  // 274
    var parts = vConstraintString.split(/ *\|\| */);                                                 // 275
    alternatives = __.map(parts, function (alt) {                                                    // 276
      if (! alt) {                                                                                   // 277
        throwVersionParserError("Invalid constraint string: " +                                      // 278
                                vConstraintString);                                                  // 279
      }                                                                                              // 280
      return parseSimpleConstraint(alt);                                                             // 281
    });                                                                                              // 282
  }                                                                                                  // 283
                                                                                                     // 284
  this.raw = vConstraintString;                                                                      // 285
  this.alternatives = alternatives;                                                                  // 286
};                                                                                                   // 287
                                                                                                     // 288
PV.parseVersionConstraint = function (constraintString) {                                            // 289
  return new PV.VersionConstraint(constraintString);                                                 // 290
};                                                                                                   // 291
                                                                                                     // 292
// A PackageConstraint consists of a package name and a version constraint.                          // 293
// Call either with args (package, versionConstraintString) or                                       // 294
// (packageConstraintString), or (package, versionConstraint).                                       // 295
// That is, ("foo", "1.2.3") or ("foo@1.2.3"), or ("foo", vc) where vc                               // 296
// is instanceof PV.VersionConstraint.                                                               // 297
PV.PackageConstraint = function (part1, part2) {                                                     // 298
  if ((typeof part1 !== "string") ||                                                                 // 299
      (part2 && (typeof part2 !== "string") &&                                                       // 300
       ! (part2 instanceof PV.VersionConstraint))) {                                                 // 301
    throw new Error("constraintString must be a string");                                            // 302
  }                                                                                                  // 303
                                                                                                     // 304
  var packageName, versionConstraint, vConstraintString;                                             // 305
  if (part2) {                                                                                       // 306
    packageName = part1;                                                                             // 307
    if (part2 instanceof PV.VersionConstraint) {                                                     // 308
      versionConstraint = part2;                                                                     // 309
    } else {                                                                                         // 310
      vConstraintString = part2;                                                                     // 311
    }                                                                                                // 312
  } else if (part1.indexOf("@") >= 0) {                                                              // 313
    // Shave off last part after @, with "a@b@c" becoming ["a@b", "c"].                              // 314
    // Validating the package name will catch extra @.                                               // 315
    var parts = part1.match(/^(.*)@([^@]*)$/).slice(1);                                              // 316
    packageName = parts[0];                                                                          // 317
    vConstraintString = parts[1];                                                                    // 318
    if (! vConstraintString) {                                                                       // 319
      throwVersionParserError(                                                                       // 320
        "Version constraint for package '" + packageName +                                           // 321
          "' cannot be empty; leave off the @ if you don't want to constrain " +                     // 322
          "the version.");                                                                           // 323
    }                                                                                                // 324
  } else {                                                                                           // 325
    packageName = part1;                                                                             // 326
    vConstraintString = "";                                                                          // 327
  }                                                                                                  // 328
                                                                                                     // 329
  PV.validatePackageName(packageName);                                                               // 330
  if (versionConstraint) {                                                                           // 331
    vConstraintString = versionConstraint.raw;                                                       // 332
  } else {                                                                                           // 333
    versionConstraint = PV.parseVersionConstraint(vConstraintString);                                // 334
  }                                                                                                  // 335
                                                                                                     // 336
  this.package = packageName;                                                                        // 337
  this.constraintString = vConstraintString;                                                         // 338
  this.versionConstraint = versionConstraint;                                                        // 339
};                                                                                                   // 340
                                                                                                     // 341
PV.PackageConstraint.prototype.toString = function () {                                              // 342
  var ret = this.package;                                                                            // 343
  if (this.constraintString) {                                                                       // 344
    ret += "@" + this.constraintString;                                                              // 345
  }                                                                                                  // 346
  return ret;                                                                                        // 347
};                                                                                                   // 348
                                                                                                     // 349
// Structure of a parsed constraint:                                                                 // 350
//                                                                                                   // 351
// /*PV.PackageConstraint*/                                                                          // 352
// { package: String,                                                                                // 353
//   constraintString: String,                                                                       // 354
//   versionConstraint: /*PV.VersionConstraint*/ {                                                   // 355
//     raw: String,                                                                                  // 356
//     alternatives: [{versionString: String|null,                                                   // 357
//                     type: String}]}}                                                              // 358
PV.parsePackageConstraint = function (part1, part2) {                                                // 359
  return new PV.PackageConstraint(part1, part2);                                                     // 360
};                                                                                                   // 361
                                                                                                     // 362
PV.validatePackageName = function (packageName, options) {                                           // 363
  options = options || {};                                                                           // 364
                                                                                                     // 365
  var badChar = packageName.match(/[^a-z0-9:.\-]/);                                                  // 366
  if (badChar) {                                                                                     // 367
    if (options.detailedColonExplanation) {                                                          // 368
      throwVersionParserError(                                                                       // 369
        "Bad character in package name: " + JSON.stringify(badChar[0]) +                             // 370
          ".\n\nPackage names can only contain lowercase ASCII alphanumerics, " +                    // 371
          "dash, or dot.\nIf you plan to publish a package, it must be " +                           // 372
          "prefixed with your\nMeteor Developer Account username and a colon.");                     // 373
    }                                                                                                // 374
    throwVersionParserError(                                                                         // 375
      "Package names can only contain lowercase ASCII alphanumerics, dash, " +                       // 376
        "dot, or colon, not " + JSON.stringify(badChar[0]) + ".");                                   // 377
  }                                                                                                  // 378
  if (!/[a-z]/.test(packageName)) {                                                                  // 379
    throwVersionParserError("Package names must contain a lowercase ASCII letter.");                 // 380
  }                                                                                                  // 381
  if (packageName[0] === '.') {                                                                      // 382
    throwVersionParserError("Package names may not begin with a dot.");                              // 383
  }                                                                                                  // 384
  if (packageName.slice(-1) === '.') {                                                               // 385
    throwVersionParserError("Package names may not end with a dot.");                                // 386
  }                                                                                                  // 387
  if (packageName.indexOf('..') >= 0) {                                                              // 388
    throwVersionParserError("Package names may not contain two consecutive dots.");                  // 389
  }                                                                                                  // 390
  if (packageName[0] === '-') {                                                                      // 391
    throwVersionParserError("Package names may not begin with a hyphen.");                           // 392
  }                                                                                                  // 393
  // (There is already a package ending with a `-` and one with two consecutive `-`                  // 394
  // in troposphere, though they both look like typos.)                                              // 395
                                                                                                     // 396
  if (packageName[0] === ":" || __.last(packageName) === ":") {                                      // 397
    throwVersionParserError("Package names may not start or end with a colon.");                     // 398
  }                                                                                                  // 399
};                                                                                                   // 400
                                                                                                     // 401
var throwVersionParserError = function (message) {                                                   // 402
  var e = new Error(message);                                                                        // 403
  e.versionParserError = true;                                                                       // 404
  throw e;                                                                                           // 405
};                                                                                                   // 406
                                                                                                     // 407
// Return true if the version constraint was invalid prior to 0.9.3                                  // 408
// (adding _ and || support)                                                                         // 409
//                                                                                                   // 410
// NOTE: this is not used on the client yet. This package is used by the                             // 411
// package server to determine what is valid.                                                        // 412
PV.invalidFirstFormatConstraint = function (validConstraint) {                                       // 413
  if (!validConstraint) return false;                                                                // 414
  // We can check this easily right now, because we introduced some new                              // 415
  // characters. Anything with those characters is invalid prior to                                  // 416
  // 0.9.3. XXX: If we ever have to go through these, we should write a more                         // 417
  // complicated regex.                                                                              // 418
  return (/_/.test(validConstraint) ||                                                               // 419
          /\|/.test(validConstraint));                                                               // 420
};                                                                                                   // 421
                                                                                                     // 422
// Remove a suffix like "+foo" if present.                                                           // 423
PV.removeBuildID = function (versionString) {                                                        // 424
  return versionString.replace(/\+.*$/, '');                                                         // 425
};                                                                                                   // 426
///////////////////////////////////////////////////////////////////////////////////////////////////////
